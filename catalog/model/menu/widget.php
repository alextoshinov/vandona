...
class ModelMenuWidget extends Model {
	...
	public function getTypes(){
		...
		return array(
			'html'              => $this->language->get( 'text_widget_html' ),
                        'collections'       => $this->language->get( 'text_widget_product_collections' ),
	 ...
	 *
	 */
	public function renderWidgetCollections(  $args, $setting, $widget_name= "" ){
		$this->load->model('catalog/product');
		$t  = array(
			'category_id'=> '',
			'limit'   => '6',
		...
		$setting = array_merge( $t, $setting );
			...
			ob_start();
			require( DIR_TEMPLATE .  $this->getLayout('collections') );
			$output = ob_get_contents();
			ob_end_clean( );
